...
//获取组合股票列表
function getuserportfoliodetail(portfolioid)
{
    $.ajax({
        url: "/api/proc_userportfolio/",
        method: 'POST',
        cache: false,
        data: {
            action: "get_userportfolio_detail",
        ...
        success: function (data) {
            ...
            if (data.errorCode == '0000000') {
                ...
                for (let i = 0; i < data.data0.length; i++) {
                    let stockinfo = {
                        portfolioid: data.data0[i][0],
                        stockcode: data.data0[i][1],
                        stockname: data.data0[i][2],
                        insertdate: data.data0[i][3],
                        orderid: data.data0[i][4]
                    }
                    cur_stocklist.push(stockinfo);
                }
                refresh_stock_list();
            }
            else {
...
function refresh_stock_list() {
    //todo
    //通过股票列表，还有显示的view获取表格明细数据，然后回填表格
    //以下代码仅为测试代码
    let tabledata = [];
    let tableColumns = ['股票代码', '价格', '变动', 'PE'];
    for (let i = 0; i < cur_stocklist.length; i++) {
        let stockinfo = cur_stocklist[i];
        //产生一个随机数当价格差额
        let priceInc= Math.random() * 10;
        let stockdata = [stockinfo.stockcode, 85.74+priceInc, priceInc/85.74 , 12.5+priceInc/85.74];
        tabledata.push(stockdata);
    }
    setPortfolioDataTable1(tabledata, tableColumns);
}

//向股票列表中添加股票
function setPortfolioDataTable1(tabledata, tableColumns) {
    //特殊设置
    //1、不显foot和head
    let domstr = '';
    ...
    */
    ...
    let tableName = 'PortfolioDataTable1';
    ...
    $('#' + tableName).dataTable({
        ...
        "destroy": true,
        "pageLength": 20000,  //不分页，超过这个数字就爆炸吧。。。。
    ...
    $('#dataTable4').dataTable({
        ...
        },
        {
            ...
            action: function (e, dt, node, config) {
                setTableColumnsData('dataTable4', tableColumns, selectedValues);
                $("#choicecolumns").modal();
            }
        }
        ],
