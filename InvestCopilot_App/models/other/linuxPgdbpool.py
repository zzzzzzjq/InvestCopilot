...
class pgsqlPool(object):
    ...
    def __new__(cls, *args, **kw):
        if not cls.__pool:
            #user, password, ip, listener, sid = getCfg()
            # user, password, ip, listener, sid = getEncodeDBParameter()
            print("socket.gethostname():",socket.gethostname())
            if socket.gethostname()=="iZebwatodogxv8Z":
                user, password, ip, listener, sid = "dsidmfactors", "gfjjdsid", '172.18.163.216', '5432', 'Inari'
            else:
                user, password, ip, listener, sid = "dsidmfactors", "gfjjdsid", 'www.daohequant.com', '5432', 'Inari'
