...
class audioMode():
    ...
    def addCallSummaryData(self,audioId,audioData):
        rst=ResultData()
        rst.data={'rowcount':-1}
        try:
            with dbmongo.Mongo("website") as md:
                mydb = md.db
                bernsteSet = mydb["Audio"]
                existsData = bernsteSet.find_one({'id': audioId})
                if existsData is None:
                    rts = bernsteSet.insert_one(audioData)
                    if rts.inserted_id is not None:
                        rst.data = {'rowcount': 1}
                    else:
                        rst.data = {'rowcount': 0}
        except:
            Logger.error("audioId:%s"%audioId)
            Logger.error(traceback.format_exc())
    ...
    def getAudioSummaryView(self,vtags=["Audio"],translation="zh",userId=None,companyIds=[],meetingTypes=[],queryTitle=None,page=1,pageSize=20):
        """
        电话会议摘要数据  分页查询
        meetingTypes  电话会议分类
        :return:
        ...
        try:
            ...
            if len(companyIds)==1:
                ...
                with dbmongo.Mongo("website") as md:
                    mydb = md.db
                    fSet = mydb["Audio"]
                    document_count = fSet.count_documents(
                        {"tickers": {"$in": companyIds}, "updateTime": {"$exists": True}})
                if document_count == 0:
                    #如果用户没有上传会议  展示系统默认会议
                    querys['id']  = {"$in":["Audio_89ce1f3497d64151ec66b4d095d7ff4a", "Audio_1ba07b9cf786be7a59694bfbd1cf1514",]}
                    visitFlag=True
                else:
            ...
            #会议分类
            if len(meetingTypes)>0:
                querys['meetingType'] = {"$in": meetingTypes}
            #标题搜索
            ...
            Logger.errLineNo(msg="getAudioSummaryView error")
