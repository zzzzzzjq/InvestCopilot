...
insert_east_hk_daymarket= """
   insert into spider.east_hk_daymarket
...
def getHkMarktet(con, cur,stocktype='stocks'):
    """
    港股实时行情同步
    """
    try:
        #当前工作日
        workDay = market_utils.getHkWorkDay()
        del_hkstockmarket = "delete from spider.east_hk_daymarket t where t.tradeDate=%s"
        hkdaymarkDF = get_realtime_quotes('港股')
        #	最新价	涨跌幅	涨跌额	成交量	成交额	换手率	动态市盈率	量比	股票代码	市场编号	股票名称	最高	最低	今开	昨日收盘	总市值	流通市值	更新时间戳	最新交易日
        hkdaymarkDF = hkdaymarkDF.replace({"-": None,np.NAN:""})
        # of="d:/work/temp/港股3.xlsx"
        hkdaymarkDF = hkdaymarkDF[hkdaymarkDF['总市值'].astype(float) > 0]
        ...
        hkMarketList=[]
        for _idx, hkdt in hkdaymarkDF.iterrows():
            windCode = str(int(hkdt.股票代码)).zfill(4) + ".HK"
            stockCode = str(hkdt.股票代码) + ".HK"
            rid ="116."+str(hkdt.股票代码)
            stockname = hkdt.股票名称
            s_dq_close = hkdt.最新价
            if pd.isnull(s_dq_close) or s_dq_close is None:
                continue  # 停牌
            s_dq_change = hkdt.涨跌额
            s_dq_pctchange = hkdt.涨跌幅
            s_dq_volume = hkdt.成交量
            s_dq_amount = hkdt.成交额
            s_dq_open = hkdt.今开
            s_dq_high = hkdt.最高
            s_dq_low = hkdt.最低
            s_dq_preclose = hkdt.昨日收盘
            tradedate = workDay
            hkMarketList.append([windCode,stockCode,rid, stockname, s_dq_close, s_dq_change, s_dq_pctchange, s_dq_volume, s_dq_amount, s_dq_open, s_dq_high, s_dq_low, s_dq_preclose, tradedate])
            # print(hkMarketList[-1])
            # cur.execute(insert_east_hk_daymarket, hkMarketList[-1])
            # print('row:', row)
        cur.execute(del_hkstockmarket, [workDay])
        cur.executemany(insert_east_hk_daymarket, hkMarketList)
        con.commit()
    except Exception as ex:
        print(traceback.format_exc())
        logging.error("通过港股行情数据异常:%s" % ex)
        return "同步通过港股行情数据异常!"

    return "同步港股行情数据成功!"


def synHkMarket(stocktype='stocks'):
    try:
        isHkWorkDay = market_utils.isHkWorkDay()
        if not isHkWorkDay:
            return
        con, cur = dbutils.getConnect()
        msg="获取港股行情数据成功"
        # 获取港股行情数据
        getHkMarktet(con, cur,stocktype)
        # 检查最新行情数据，发送邮件
        if stocktype in ['stocks']:
            q_hkMarket = 'select count(1) allnum,tradedate from spider.east_hk_daymarket t where t.tradedate =(select max(tradedate) from spider.east_hk_daymarket) group by tradedate'
        elif stocktype in ['idx']:
            q_hkMarket = 'select count(1) allnum,tradedate from spider.east_hk_idxmarket t where t.tradedate =(select max(tradedate) from spider.east_hk_idxmarket) group by tradedate'
        else:
            return
        # mkDF = pd.read_sql(q_hkMarket, con)
        cur.execute(q_hkMarket)
        rsData = cur.fetchall()
        if len(rsData)==0:#mkDF.empty:
            title = "获取港股行情数据为空，请手动触发！"
        else:
            allNum = rsData[0][0]
            tradeDate = rsData[0][1]
            #copy
            if stocktype in ['stocks']:
                d_his="delete  from  spider.east_hk_stockprice  where tradedate=%s"
                copy="insert into spider.east_hk_stockprice  select  windcode,stockcode, ric, s_dq_close, s_dq_change, s_dq_pctchange, s_dq_volume, s_dq_amount, s_dq_open, s_dq_high, s_dq_low, s_dq_preclose, tradedate from spider.east_hk_daymarket" \
                     " where tradedate= (select max(tradedate) from spider.east_hk_daymarket) and windcode in (select windcode from spider.east_hk_stockprice  group by windcode )"
            else:
                d_his = "delete  from  spider.east_hk_idxprice  where tradedate=%s"
                copy = "insert into spider.east_hk_idxprice  select  windcode,stockcode, ric, s_dq_close, s_dq_change, s_dq_pctchange, s_dq_volume, s_dq_amount, s_dq_open, s_dq_high, s_dq_low, s_dq_preclose, tradedate from spider.east_hk_daymarket" \
                       " where tradedate= (select max(tradedate) from spider.east_hk_idxmarket) and windcode in (select windcode from spider.east_hk_idxprice  group by windcode )"
            cur.execute(d_his,[tradeDate])
            cur.execute(copy)
            con.commit()
            copynum=cur.rowcount
            title = "生产环境港股[%s]行情抓取：[%s]日获取数据[%d]条,copy[%s]条[%s]" % (stocktype,tradeDate, allNum ,copynum,datetime.datetime.now())

            first_hisdata(stocktype='idx', beg=tradeDate)
        wx_send.send_wx_msg_operation(title)
        print("sync hkMarktet" , title)
    except:
...
insert_eaststockmarket = """
   insert into spider.east_hk_stockprice
...
insert_eastidxkmarket = """
   insert into spider.east_hk_idxprice
   (windcode,stockcode, ric, s_dq_close, s_dq_change, s_dq_pctchange, s_dq_volume, s_dq_amount, s_dq_open, s_dq_high, s_dq_low, s_dq_preclose, tradedate)
 values
   (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s)
"""
...
def getStockHisMarket(windCode,stockCode,eastCode='1.510050',beg="",end="",stocktype="stocks"):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if stocktype in ['stocks']:
                del_hkstockmarket = "delete from spider.east_hk_stockprice t where t.tradeDate>=%s and windCode=%s"
            elif stocktype in ['idx']:
                del_hkstockmarket = "delete from spider.east_hk_idxprice t where t.tradeDate>=%s and windCode=%s"
            ...
            con,cur = dbutils.getConnect()
            #(stockcode, stockname, s_dq_close, s_dq_change, s_dq_pctchange, s_dq_volume, s_dq_amount, s_dq_open, s_dq_high, s_dq_low, s_dq_preclose, tradedate)
            # print("dataDF:",del_hkstockmarket,[tradeDate,stockCode])
            dataDF=dataDF[['windCode','stockCode','ric','收盘','涨跌额','涨跌幅','成交量','成交额','今开','最高','最低','昨收','交易日期']]
            cur.execute(del_hkstockmarket,[tradeDate,windCode])
...
def first_hisdata(stocktype='stocks',beg="20170101"):
    # getStockHisMarket(beg="0",end='20200920')
    lcon,lcur=dbutils.getConnect()
    # # qhkcodedf = pd.read_sql("select windcode,stockcode,ric from newdata.a_reuters_basic arb"
    # #                         " where exchangecountry ='Hong Kong' and windcode not in (select windcode from   spider.east_hk_stockprice group by windcode  )",lcon)
    # qhkcodedf = pd.read_sql("select windcode,stockcode,ric from newdata.a_reuters_basic arb"
    #                         " where exchangecountry ='Hong Kong'",lcon)
    bt=datetime.datetime.now()
    #
    #
    # allsize=len(qhkcodedf)
    # for qc in qhkcodedf.itertuples():
    #     windcode=qc.windcode

    # qCHcodedf = pd.read_sql(
    #     "select * from config.chstocks c WHERE C.WINDCODE NOT IN (select windcode from   spider.east_CH_stockprice group by windcode) order by ord",
    #     lcon)
    if stocktype in ['stocks']:
        qsql="select * from config.hkstocks c WHERE C.WINDCODE NOT IN (select windcode from   spider.east_hk_stockprice group by windcode)  order by ord"
    elif stocktype in ['idx']:
        qsql="select * from config.stockinfo c WHERE stocktype='idx' and area='HK'"
    else:
        return
    qCHcodedf = pd.read_sql(
        qsql,
    ...
    for qc in qCHcodedf.itertuples():
        ...
        if stocktype in ['idx']:
            ...
            getStockHisMarket(windCode=windcode, stockCode=str(windcode).zfill(5),
                              eastCode='116.%s' % (str(windcode).split(".")[0]).zfill(5), beg=beg, end="",
                              stocktype=stocktype)
        # getStockHisMarket(windCode='0700.HK',stockCode='00700.HK',eastCode='116.00700', beg="20170101", end="")
        # time.sleep(random.randint(3,8))
        allsize-=1
        print(windcode,"allsize:%s"%allsize)
    et = datetime.datetime.now()
    print(bt)
    print(et)
    # getStockHisMarket(stockCode='1.510050', stockName='上证50ETF', beg="20210319", end="")
    # getStockHisMarket(stockCode='100.HSI',stockName='恒生指数',beg="20210319",end="")
    # getStockHisMarket(stockCode='100.HSCEI',stockName='恒生国企指数',beg="20210319",end="")
    # getStockHisMarket(stockCode='1.204001',stockName='GC001',beg="20210319",end="")
    # getStockHisMarket(stockCode='100.NDX100',stockName='纳斯达克100',beg="20210319",end="")
    # getStockHisMarket(stockCode='105.NDAQ',stockName='纳斯达克',beg="20210319",end="")
    pass
if __name__ == '__main__':
    # first_hisdata(stocktype='idx')
    synHkMarket(stocktype='stocks')
    # synHkMarket(stocktype='idx')
    #"0728.HK" 1088.HK
    # fcode="0728.HK"
    # getStockHisMarket(windCode=fcode, stockCode=str(fcode).zfill(5),
    #                   eastCode='116.%s' % (str(fcode).split(".")[0]).zfill(5), beg="20170101", end="")
    pass